...
select * from record
insert into  record values(58,"management","married","tertiary","no",2143,"yes","no","unknown",5,"may",261,1,-1,0,"unknown","no")

insert into record values(44,"technician","single","secondary","no",29,"yes","no","unknown",5,"may",151,1,-1,0,"unknown","no"),
...
create table if not exists  record(age int,
job varchar(30),
marital varchar(30),
education varchar(30),
`default` varchar(30),
...
select age from record
select age,loan from record
// to select a column which is on keyword name-----use (``)
select 	`default` from record

select * from record 

// to select specific from from a column---
 //to starting 4 rows
select * from record limit 4

// to select 4 rows after 5 rows
select * from record limit 5,4

select * from record where age>35
select * from record where age>40 and job="retired"
select * from record where balance<500 and (education='unknown' or marital='single')

// to select types of a specific columns
select (distinct(job)) from record
select count(distinct(job)) from record

// ordering of a table by deafult its in ascending--------

select * from record order by job
select * from record order by job desc


// questions or task on basic of mysql query------
1- with this data try to find the sum of balanc.
select sum(balance) from record

2- find the average of balance.
select avg(balance) from record

3- who is having a min balance 
select * from record order by balance limit 1
select * from record where balance in(select min(balance) from record)
select * from record where balance= (select min(balance) from record)

4- try out the person who is having loan 
select * from record where loan='yes'

5- try to find out the avg balance for all the people whose job  role is admin--------
select avg(balance) from record where job='admin.'

6-try out to find  a record without job whose age is below 45------
select * from record where job='unknown' and age<45

7-try out to find out a record where education is primarty and person is jobless----
select * from record where education='primary' and job='unknown'

8- try out to find a record which is having balance is negative 
select * from record where balance<0

9- try out to find  a record who is not having house at all along with there balance 
select balance,housing from record where housing='no'

use bank

//  STORE PROCEDURES :-- its is bascially equivalent to the functions 
----- it is use to create once use many 
---- syntax:---
...
// we can call procedure into another file also

// parameterise in procedures-------

// syntax   ------

delimiter &&
create procedure  call_job_avg( in A varchar(20))
begin
select avg(balance) from record where job=A;
end &&

call call_job_avg('unknown')

//---- for two or parameter procedures------

delimiter &&
create procedure multi_parameter(in A varchar(20),in B int(10))
begin

select * from record where job=A and age<B;
end &&


call multi_parameter('unknown',45)
...
delimiter &&
create procedure multi_para(in A varchar(20),in B varchar(10))
begin

select * from record where education=A and job=B;
end &&


call multi_para('primary','unknown')



// VIEW --- OF A TABLE---
// ITS SIMILAR TO A TABLE, IT IS A SUBSET of table out of whole data 

// syntax -------

create view subtable as select age,job,marital,balance from record

select * from subtable

// CREATING VIEW WITH WHERE and also with other operation---------------

create view subtable_with_where as select age,job,marital,balance from record where age>44

select * from  subtable_with_where
